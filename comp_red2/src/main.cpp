...
void intake(){
    spiner.spin(vex::directionType::fwd,100 / speed2,vex::velocityUnits::pct);
    spiner2.spin(vex::directionType::fwd,100 / speed2,vex::velocityUnits::pct);
}
void outtake(){
  ...
  while(abs == true){
    Vision.takeSnapshot(Vision__ORNAGES);
            if(Vision.objects[0].exists){
              if(Vision.objects[0].centerX > 145 && Vision.objects[0].centerX < 170){
              ...
              if(Vision.objects[0].centerX < 145){
                Left.spin(reverse);
                Right.spin(forward);
              }
              if(Vision.objects[0].centerX > 170){
